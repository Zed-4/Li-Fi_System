...
def checkSensors():
    while(True):
        if ( lps.temperature > TEMP_THRESHOLD and not pygame.mixer.music.get_busy()):
            ...
            pygame.mixer.music.play()
        if ( lps.pressure > PRESSURE_THRESHOLD and not pygame.mixer.music.get_busy()):
            pygame.mixer.music.load("./LiFiSounds/PressureSensorAlert.wav")
            pygame.mixer.music.play()
        if ( get_humidity() > HUMIDITY_THRESHOLD and not pygame.mixer.music.get_busy()):
...
main_thread.start()
safety_thread = Thread(target=checkSensors)
safety_thread.start()
window.mainloop()
